"""Collection of parsers for machine learning. Updated for BigTables with deeper nesting."""
import re
...
def groupby_looper2(bhsac_df):
    ...
    Updated: Use 'mother' for parents to enable deeper nesting."""
    # Groupby looping
    ...
    for verse_count, g in enumerate(gby):
        ...
        sent_word_nodes = []
        sent_gram_nodes = []
        sent_parent_nodes = []  # Now using "mother" for deeper nesting
        sent_trope_nodes = []
        word = ""

        for i, row in verse.iterrows():
            # Grammar with rela/typ for extra info
            if row.otype == "clause":
                sent_gram.append(row.rela or row.kind)  # Use rela if available
                sent_gram_nodes.append(row.n)
                sent_parent_nodes.append(row.mother)  # Switch to "mother" for hierarchy
...
def groupby_looper3(bhsac_df):
    """Enhanced looper with additional sections: includes mother, rela, typ for each verse."""
    pattern = "[" + "".join(utils.trops) + "]"
    gby = bhsac_df.groupby(by="in.verse")
    ...
    for verse_count, g in enumerate(gby):
        ...
        sent_gram = []
        sent_trope = []
        sent_mother = []  # New: mother IDs
        sent_rela = []    # New: relations
        sent_typ = []     # New: types
        ...
        for i, row in verse.iterrows():
            ...
            if row.otype == "clause":
                ...
                sent_mother.append(int(row.mother) if pd.notna(row.mother) else None)
...
def compile_sets(lines, num_samples=None, use_pasuk=False):
    ...
    for line in lines[:num_samples]:
        pasuk, pasuk_parts, input_text, target_text, _, _, _, _, _, _, _ = line  # Adjusted for extra fields
        if use_pasuk:
